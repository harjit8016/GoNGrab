...
function packCategoriesIntoColumns(categoryGroups, numCols = 5) {
  ...
  const catWeights = categories.map(name => ({
    ...
    weight: 2.8 + categoryGroups[name].length
...
function sanitizeAndCleanSvg(svgStr) {
  if (!svgStr || typeof svgStr !== 'string') return '';
  let cleaned = svgStr;
  // Dynamically remove full-canvas background rect elements that cause square bounding box artifacts
  cleaned = cleaned.replace(/<rect\s+[^>]*?(?:width=["'](?:100%|100|500|1000)["'])[^>]*?\/?>/gi, (match) => {
    if (match.includes('x="0"') || match.includes('y="0"') || match.includes('width="100%"') || match.includes('width="100"')) {
      return '';
    }
    return match;
  });
  return cleaned.replace('<svg ', '<svg class="cat-title-svg cat-animated-svg" style="background: transparent !important; overflow: visible !important;" ');
}
...
function getCategorySvg(categoryName, customSvg) {
  ...
  if (finalSvg && typeof finalSvg === 'string' && finalSvg.includes('<svg')) {
    return sanitizeAndCleanSvg(finalSvg);
  }
  ...
  if (!preset) return '';
  return sanitizeAndCleanSvg(preset);
}
